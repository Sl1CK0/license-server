  ...
  LicenseKey.fetch = async (key) => {
    ...
    if (data) {
      try {
        data.issueDate = data.issueDate ? new Date(parseInt(data.issueDate, 10)).toLocaleDateString() : 'N/A';
        data.startDate = data.startDate ? new Date(parseInt(data.startDate, 10)).toLocaleDateString() : 'N/A';
        data.endDate = data.endDate ? new Date(parseInt(data.endDate, 10)).toLocaleDateString() : 'N/A';
      } catch (e) {
